...
fn version(mut cx: FunctionContext) -> JsResult<JsString> {
	Ok(cx.string(format!("Vanilla version: v{}.{}.{}", VERSION_MAX, VERSION_MID, VERSION_MIN)))
}
...
fn merge_files(mut cx: FunctionContext) -> JsResult<JsBoolean> {
	...
	let merge_res = merge_files::merge(&mergeable_files, base_path.as_str(), new_path.as_str(), out_path.as_str()); 
	if merge_res.is_err() {
		return Ok(cx.boolean(false))
	}
