...
pub fn merge(file_types: &Vec<MergeableFile>, base_path: &str, additive_path: &str, out_path: &str) -> Result<(), MergeError> {
	...
	let base_name = Path::new(base_path).file_name().unwrap().to_str();
	if base_name.is_none() {
		return Err(MergeError::BadFilePath);
	}
	let base_name = Path::new(base_path).file_name().unwrap().to_str().unwrap();
	let additive_name = Path::new(additive_path).file_name();
	if additive_name.is_none() {
		return Err(MergeError::BadFilePath);
	}
	let additive_name = Path::new(additive_path).file_name().unwrap().to_str();
	if additive_name.is_none() {
		return Err(MergeError::BadFilePath);
	}
	let additive_name = Path::new(additive_path).file_name().unwrap().to_str().unwrap();
